...
case $OPTION in
	1)
		...
		--pid-path=/var/www/run/nginx.pid \
		--lock-path=/var/www/run/nginx.lock \
		--http-client-body-temp-path=/var/www/cache/nginx/client_temp \
		--http-proxy-temp-path=/var/www/cache/nginx/proxy_temp \
		--http-fastcgi-temp-path=/var/www/cache/nginx/fastcgi_temp \
		--user=nginx \
		--group=nginx \
		--with-cc-opt=-Wno-deprecated-declarations"

                 # XP REMOVED --without-http_geo_module
                 # for nginx-ultimate-bad-bot-blocker
                 # enabled by default (./configure: error: invalid option "--with-http_geo_module")
	...
	2) # Uninstall Nginx
		...
		if [[ "$LOGS" = 'y' ]]; then
			...
			rm -r /var/www/log >> /tmp/nginx-autoinstall.log 2>&1
